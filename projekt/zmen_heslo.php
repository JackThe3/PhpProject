...
<h1>Zmena hesla</h1>
<?php
//zmena hesla používateˇˇla
if (isset($_SESSION['user'])) {
	if (isset($_POST["submit"])) {
		if (isset($_POST["stare_heslo"]) && (trim($_POST["stare_heslo"]) != '') && ($pouzivatel = over_pouzivatela($mysqli, $_SESSION["user"], $_POST["stare_heslo"]))) {
		
			if (isset($_POST["heslo"]) && isset($_POST["heslo2"]) && nazov_ok($_POST["heslo"]) && ($_POST["heslo"] == $_POST["heslo2"])) {
				zmen_heslo($mysqli, $_SESSION['id_pouz'], $_POST["heslo"]);     
			}
